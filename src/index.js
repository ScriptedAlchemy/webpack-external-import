  ...
  if (
    ...
    window.entryManifest[nameSpace][module].dependencies.forEach(file => {
      if (!__webpack_modules__[file.id]) {
        file.sourceFiles.forEach(chunkFile =>
          dependencyPaths.push(basePath + chunkFile)
        );
      }
    });
  } else if (
    ...
    window.entryManifest[nameSpace][`${module}.js`].dependencies.forEach(
      file => {
        if (!__webpack_modules__[file.id]) {
          file.sourceFiles.forEach(chunkFile =>
            dependencyPaths.push(urljoin(basePath, chunkFile))
          );
        }
      }
    );
...
export { corsImport, ExternalComponent };
